...
<div id="user-panel">
    ...
    <div id="action-buttons" :class="{'no-clicked': user.login!==action.login}" v-else>
        <button class="hovered but txt-blue action-button but-check" @click="check" :class="{'no-clicked': called !== 0}">Check</button>
        <button class="hovered but txt-red action-button but-fold" @click="fold">Fold<div class="buttons-value">{{game.gamersData[user.login] && game.gamersData[user.login].totalBet || 0}}</div>
        </button>
        <button class="hovered but txt-yellow action-button but-call" :class="{'no-clicked': called === 0}" @click="call">Call:<div class="buttons-value">{{called}}</div>
        </button>
          <button class="hovered but txt-green action-button but-raise" @click="raise">{{txtRaise}} to:
